...
| 18 | `invalidItems` | `bf-empty-title` (title is `"   "`, empty after trim — `reason: "empty title"`); `bf-dup-id`'s second occurrence (same id planted twice — first occurrence wins, deterministically, by input order — `reason: "duplicate id"`); `bf-status-nonstring` (`status: 42`, a number — fails the shape gate before `mapStatus` ever runs — `reason: "unusable field type"`); `bf-description-nonstring` (`description: 42`, a number — `load.ts` binds `description` straight into a SQL `INSERT`, exactly as type-sensitive as a method call, so this fails the shape gate the same way — `reason: "unusable field type"`) | 4 | same |
...
| Parent chain with no epic anywhere on it (`epicEdgesDropped`'s second cause — reachable in any beads project that never used `epic`/`milestone` types) | Not planted in this fixture — every task-typed chain here eventually reaches an epic (or a cycle) | Proven directly by `beads-transform.test.ts`'s "reports a parent chain with no epic anywhere and leaves the items parentless" test; this fixture only exercises `epicEdgesDropped`'s other cause (#8 above) |
