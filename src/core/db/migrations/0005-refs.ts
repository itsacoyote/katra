/**
 * External references: `refs` + `task_refs`, and the events rebuild that
 * accepts linking them.
 *
 * Two new tables (F7 requirement 1) and one table rebuilt in place — the same
 * shape as migration 0003's rebuild, and for the same reason: SQLite has no
 * `ALTER TABLE ... ALTER COLUMN` and cannot widen a `CHECK` constraint, so
 * accepting `ref-linked`/`ref-unlinked` in `events.type` means creating the
 * new shape, copying every row across, dropping the old table and renaming
 * the new one in.
 *
 * Built from a `Sets` object like `0001-init.ts`, `0002-events-and-notes.ts`
 * and `0003-claims-and-presence.ts`, for the same reason: a `.sql` file
...
export interface RefsSets {
  readonly eventTypes: readonly string[];
}

export const DEFAULT_REFS_SETS: RefsSets = {
  eventTypes: EVENT_TYPES,
};

/** Renders migration 0005's DDL for the given value sets. */
export function buildRefsDdl(sets: RefsSets = DEFAULT_REFS_SETS): string {
  return `
