/**
 * The FTS5 search index (ADR-013): two external-content virtual tables over
 * `tasks` and `notes`, kept in sync by triggers, backfilled once for rows
 * that predate the migration.
 *
 * Unlike `0001-init.ts`, `0002-events-and-notes.ts` and
 * `0003-claims-and-presence.ts`, this migration takes **no `Sets` argument**.
 * Those three parameterise their DDL because a `CHECK` constraint mirrors a
 * TypeScript enum array (`LEVELS`, `LANES`, `EVENT_TYPES`, ...) that a `.sql`
 * file cannot reference — the Sets object is what lets a test inject a value
 * no hardcoded list could know about, proving the constraint is *generated*
 * rather than copied (see `0001-init.ts`'s docstring). Nothing here is
 * enum-derived: `tasks_fts` and `notes_fts` are fixed structural DDL — table
 * names, column names, and the trigger triad — with no `CHECK` constraint and
 * no enum in sight. There is nothing for a Sets object to parameterise, so
 * `buildSearchIndexDdl` takes none.
 */

import type { Migration } from "../migrate.js";

/** Renders migration 0004's DDL: the two FTS5 indexes, their sync triggers, and the backfill. */
export function buildSearchIndexDdl(): string {
  return `
