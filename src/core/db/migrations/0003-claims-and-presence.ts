/**
 * Claims, presence, and the events rebuild F4's claims force.
 *
 * Two new tables (spec requirements 1 and 2) and one table rebuilt in place.
 * SQLite has no `ALTER TABLE ... ALTER COLUMN` and cannot widen a `CHECK`
 * constraint, so accepting `claimed`/`released` in `events.type` — and adding
 * `prior_actor` alongside it — means creating the new shape, copying every
 * row across, dropping the old table and renaming the new one in.
 *
 * Built from a `Sets` object like `0001-init.ts` and `0002-events-and-notes.ts`,
 * for the same reason: a `.sql` file cannot reference a TypeScript array, and
 * `eventTypes` is a parameter rather than a direct `EVENT_TYPES` import so a
 * test can inject a value no hardcoded list could know about.
 */

import { EVENT_TYPES, sqlEnum } from "../../enums.js";
import type { Migration } from "../migrate.js";

export interface ClaimsAndPresenceSets {
  readonly eventTypes: readonly string[];
}

export const DEFAULT_CLAIMS_AND_PRESENCE_SETS: ClaimsAndPresenceSets = {
  eventTypes: EVENT_TYPES,
};

/** Renders migration 0003's DDL for the given value sets. */
export function buildClaimsAndPresenceDdl(
  sets: ClaimsAndPresenceSets = DEFAULT_CLAIMS_AND_PRESENCE_SETS,
): string {
  return `
...
export const migration0003: Migration = {
  version: 3,
  name: "claims-and-presence",
  sql: buildClaimsAndPresenceDdl(),
};
