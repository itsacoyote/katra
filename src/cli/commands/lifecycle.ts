...
function formatLifecycle(result: LifecycleResult): string {
  ...
  if (result.task.closeReason !== null) lines.push(`  reason  ${oneLine(result.task.closeReason)}`);
  ...
  if (result.reblocked.length > 0) {
    ...
    for (const task of result.reblocked) lines.push(`    ${task.id}  ${oneLine(task.title)}`);
  }
  return lines.join("\n");
}
