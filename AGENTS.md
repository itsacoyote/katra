...
**Status: pre-alpha.** The core tracker is built and tested — tasks, epics, dependencies, links, an append-only event stream, typed notes, cross-worktree claims and presence, the two orientation reads, a beads (`bd`) migration converter, full-text search with structured filters, activity-sorted and staleness reads, and twenty-two commands over them (`init add show list update close cancel reopen delete dep link next log note brief board claim release migrate search recent stale`). Everything else in the spec is not: external refs and snapshots. Don't assume a command works because the spec describes it — check `src/cli/commands/`, which is the complete list.
