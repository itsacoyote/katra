...
- **The 30-second freshness window can lag a branch rename by up to its own
  ...
  window bumps it. "skips the write and the branch spawn when the row is
  fresh" (`presence.test.ts`) proves the skip half; the pickup half — that a
  later, non-skipped bump actually updates `branch` — is not independently
  tested today (see row 7 above).
