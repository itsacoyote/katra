...
- **Structured filters, same command** — `--status <lane> --kind <k> --level <epic|task> --epic <id> --tag <t> --updated-before/after <when>`, usable *with or without* text. Both real examples are filter queries, not text: "what was I working on" = recently-touched, sorted by activity; "an old task lying around" = open with no recent activity.
