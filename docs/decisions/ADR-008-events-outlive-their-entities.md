...
**Copy the entity's title onto the event so history reads without a join.** Rejected for now — it denormalises for one read's convenience, and the `deleted` event already carries the title in its `reason`. Worth revisiting if `board` finds itself joining against absent rows constantly.
