...
| 3 | `brief --full` prints a body the default run truncated | `brief.test.ts` (core) — "does not truncate under --full"; `brief.test.ts` (cli) — "prints the whole body under --full" | ✅ the CLI test asserts both directions in one run |
| 4 | `brief <epic>` groups children by lane and includes a **child's event** | `brief.test.ts` (core) — "groups an epic's children by lane, in lane order", "includes a child's event in the epic's activity" | ✅ |
...
| 7 | `board --digest` leads with the newest handoff across all tasks | `board.test.ts` (cli) — "leads with the newest handoff, labelled with its lane"; `notes.test.ts` — "returns the newest handoff across every task" | ✅ asserts the digest appears *before* the counts header, so rendering it anywhere else fails |
...
| The digest survives a finished task | `board.test.ts` (cli) — "shows a digest handoff from a Done task without implying it is live"; `notes.test.ts` — "returns a handoff on a Done task rather than skipping it" | "I finished X, next is Y" is the commonest real handoff and lives on `Done` work. Filtering would hide the best ones, so the lane disambiguates instead |
