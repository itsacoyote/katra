...
| The blocker batch survives a large section | `deps.ts` — chunked at 500 ids | Measured against the bundled SQLite: 32,766 bound variables bind, 32,767 throws `too many SQL variables`. `--limit` permits a million, so an unchunked `IN (…)` turned a slow board into exit 4 |
| A handoff is counted once | `brief.test.ts` (core) — "does not say both '1 handoff' and '1 more handoff'" | Two mechanisms owned the `handoff` kind, and both fired in the one skew `briefEntity` documents as safe — printing a contradiction rather than the extra line the docstring promised |
